...
class UserDetailsAdapter: RecyclerView.Adapter<UserDetailsAdapter.MyViewHolder> , Filterable {

    public var userDetailsModelArrayList: ArrayList<UserDetailsModel>
    private val context: Context
    private lateinit var binding: UserBookingDetailsItemBinding
    private var filterList: ArrayList<UserDetailsModel>
    private val uniquePickUpDates = ArrayList<String>()

    private var filter: UserDetailsFilter? = null

    constructor(userDetailsModelArrayList: ArrayList<UserDetailsModel>, context: Context){
        this.context = context

        userDetailsModelArrayList.sortWith(compareBy({ it.pickUpDate }, { it.pickUpTime }))

        // Populate the uniquePickUpDates list
        userDetailsModelArrayList.forEach { userDetailsModel ->
            if (!uniquePickUpDates.contains(userDetailsModel.pickUpDate)) {
                uniquePickUpDates.add(userDetailsModel.pickUpDate)
            }
        }

        this.userDetailsModelArrayList = userDetailsModelArrayList
        this.filterList = userDetailsModelArrayList
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UserDetailsAdapter.MyViewHolder {

        binding = UserBookingDetailsItemBinding.inflate(LayoutInflater.from(context), parent, false)

        return MyViewHolder(binding.root)
    }

    override fun onBindViewHolder(holder: UserDetailsAdapter.MyViewHolder, position: Int) {
        val userDetailsModel = userDetailsModelArrayList[position]
        ...
        userRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(userSnapshot: DataSnapshot) {
                if (userSnapshot.exists()) {
                    ...
                    if (uniquePickUpDates.contains(userDetailsModel.pickUpDate) &&
                        currentPosition == userDetailsModelArrayList.indexOfFirst { it.pickUpDate == userDetailsModel.pickUpDate }) {
    ...
    private fun preBookingFragment(
        ...
        holder: UserDetailsAdapter.MyViewHolder
    ) {
    ...
    override fun getItemCount(): Int {
        ...
        return userDetailsModelArrayList.size
    }
    ...
    inner class MyViewHolder(itemView : View) : RecyclerView.ViewHolder(itemView){
        ...
        var itemView : CardView = binding.userBookingDetailsItemCardView
    }

    override fun getFilter(): Filter {
        if(filter == null){
            filter = UserDetailsFilter(filterList, this)
        }
        return filter as UserDetailsFilter
    }

    fun sortByPickUpDateTime() {
        userDetailsModelArrayList.sortWith(compareBy<UserDetailsModel> {
    ...
    fun sortDescendingByPickUpDateTime() {
        userDetailsModelArrayList.sortWith(compareByDescending<UserDetailsModel> {
            convertDateToSortableFormat(it.pickUpDate)
